...
def main() -> int:
    ...
    cryst = Tm > T_ONSET_C
    print(
        f"Seed flash plan: {args.n} conditions over V=[{V_LO:.0f},{V_HI:.0f}] V, "
    ...
    for i in range(args.n):
        print(
            f" {i + 1:2d}   {V[i]:4d}   {t[i]:5.1f}    {Tm[i]:8.0f}       "
    ...
    vv = np.linspace(V_LO, V_HI, 160)
    tg = np.linspace(T_LO, T_HI, 160)
    ...
    for i in range(args.n):
        ...
        ax.scatter(
            V[i],
            t[i],
            c="white" if cryst[i] else "none",
            marker=mk,
            s=120,
            edgecolors="cyan" if cryst[i] else "white",
            linewidths=1.8,
            zorder=5,
        )
        ax.annotate(
            ...
            ha="center",
            va="center",
            color="k" if cryst[i] else "white",
        )
    ax.set_xlabel("flash voltage V (V)")
    ax.set_ylabel("flash time t (ms)")
    ax.set_title(
        f"{args.n} seed flash conditions on the calibrated Tmax field\n"
