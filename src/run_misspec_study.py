...
def main() -> int:
    ...
    print(f"Misspecification stress test (Scenario A, {args.readout}, n={args.n}, "
    ...
    ax1.plot(d, [r["p_family"] for r in rows], "o-", color="#1f6fb2",
             label="P(identify family)")
    ...
    ax2 = ax1.twinx()
    ax2.plot(d, [r["median_ea_err"] for r in rows], "s--", color="#d1772b",
             label="median |Ea_eff error|")
    ax2.set_ylabel("median |Ea_eff error| (eV)", color="#d1772b", fontsize=12)
    ax2.tick_params(axis="y", labelcolor="#d1772b")
    ax1.set_title("Misspecification stress test: recovery vs wrong thermal model\n"
