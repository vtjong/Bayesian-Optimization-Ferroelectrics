...
def vis_pred(train_x, train_y, test_grid, obs, nshape):
...
def vis_acq(train_x, train_y, test_grid, pred_labels, upper_surf, lower_surf, ucb, th, pi, ei, ca):
    fig = go.Figure(data=[go.Surface(z = upper_surf, x=test_grid[:,0], 
                    y=test_grid[:,1], opacity=0.5, showscale=False)])
    fig.add_trace(go.Surface(z = lower_surf, x=test_grid[:,0], y=test_grid[:,1], 
