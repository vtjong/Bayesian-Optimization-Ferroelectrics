...
simplest. Charts:
    ...
    (TB, t)           -- unweighted thermal budget
    (log TBac(Ea), t) -- activated budget, one chart per trial Ea (an Ea-indexed family)
...
def _raw_descriptors(V: np.ndarray, t: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    return {"Tmax": Tmax, "TB": TB, "t": np.asarray(t, float),
            "V": np.asarray(V, float), **{f"logTBac_{ea}": logTBac[ea] for ea in EA_GRID}}
