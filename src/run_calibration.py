...
def load_onset():
    ...
    if fla.exists():
        d = pd.read_csv(fla, index_col=0); d.columns = [c.strip() for c in d.columns]
        T += list(d["Max temperature (degC)"]); P += list(d["2Pr (uC/cm2), Pristine state"])
        src += ["flash"] * len(d)
    rta = DATA / "FE_HZCO_samples_210311.csv"
    if rta.exists():
        d = pd.read_csv(rta, index_col=0)
        m = d["2Pr (uC/cm2), Pristine state"].notna()
        T += list(d["RTA temperature (C)"][m]); P += list(d["2Pr (uC/cm2), Pristine state"][m])
        src += ["rta"] * int(m.sum())
...
def load_flash_thermal():
    ...
    if not f.exists():
        return None
    d = pd.read_csv(f, index_col=0); d.columns = [c.strip() for c in d.columns]
    return (d["Flash voltage (kV)"].values.astype(float),
...
def fit_thermal(V, t, Tm):
    ...
    return (float(np.exp(coef[0])), float(coef[1]), float(coef[2])), pred, ss, \
...
def _eps_from_loop(f):
    ...
    if len(V) < 50:
        return None
    vmax = np.nanmax(np.abs(V)); tail = np.abs(V) > 0.8 * vmax
    if tail.sum() < 10:
        return None
    ...
    return dict(eps_r=eps_r, twoPr=twoPr, series=f.split("/")[-2],
...
def eps_stats(rows):
    eps = np.array([r["eps_r"] for r in rows]); p2 = np.array([r["twoPr"] for r in rows])
    ...
    return dict(corr=float(np.corrcoef(eps, p2)[0, 1]),
                floor=float(np.mean(floor)) if len(floor) else np.nan,
                floor_sd=float(np.std(floor)) if len(floor) else np.nan,
                n_floor=int(len(floor)), rep_cv=float(np.median(cvs)) if cvs else np.nan,
                eps=eps, p2=p2)
...
def main():
    ...
    print(f"  corr(eps_r,2Pr)={es['corr']:.2f}   amorphous floor eps_r={es['floor']:.0f}"
    ...
    for s, col, mk in [("flash", "#d1772b", "o"), ("rta", "#1f6fb2", "s")]:
        mask = src == s
        ax[0].scatter(T[mask], P[mask], c=col, marker=mk, s=42, edgecolors="k",
                      lw=0.4, label=f"{s}: 2Pr vs T")
        if s in onset:
            t50, w, ofn = onset[s]
            ...
            ax[0].text(t50 + 4, P.max() * (0.9 if s == "flash" else 0.75),
                       f"{s} T50={t50:.0f}C", color=col, fontsize=9, fontweight="bold")
    ax[0].set_xlabel("peak (flash) / anneal (RTA) temperature (C)")
    ax[0].set_ylabel("2Pr (uC/cm2)")
    ax[0].set_title("1. Crystallization onset ~370-390 C\n(two independent datasets)",
                    fontweight="bold", fontsize=11); ax[0].legend(fontsize=8)
    ...
    ax[1].set_xlabel("measured IR Tmax (C)"); ax[1].set_ylabel("model Tmax(V,t) (C)")
    ax[1].set_title(f"2. Thermal anchor Tmax(V,t)\nR2={r2:.2f}, MAE={mae:.0f} C",
                    fontweight="bold", fontsize=11)
    # panel 3: permittivity
    ax[2].scatter(es["p2"], es["eps"], c="#2e8b57", s=30, edgecolors="k", lw=0.3, alpha=0.8)
    ax[2].axhspan(es["floor"] - es["floor_sd"], es["floor"] + es["floor_sd"],
                  color="gray", alpha=0.25)
    ax[2].axhline(es["floor"], color="gray", lw=1.5, ls="--",
                  label=f"amorphous floor eps_r~{es['floor']:.0f}")
    ax[2].set_xlabel("2Pr (uC/cm2)  ->  crystallinity"); ax[2].set_ylabel("effective eps_r (dQ/dV)")
    ax[2].set_title(f"3. Permittivity tracks crystallinity\nr={es['corr']:.2f} "
                    "(large-signal; needs C-V)", fontweight="bold", fontsize=11)
