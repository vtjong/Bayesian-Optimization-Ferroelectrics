...
from discovery.functional import TEMP_CENTERS, fit_weighting, make_window_dataset, occupancy
...
def main() -> int:
    ...
    Va, ta, ya = make_dataset(args.n, SCENARIOS["A"], args.readout, np.random.default_rng(1))
    m_a = compare(build_charts(Va, ta), ya, args.readout)["margin_over_vt"]

    print(
        f"Functional regression (fit R2={r2:.2f}): learned weighting peaks at "
        f"{peakT:.0f} C  (planted window {args.lo:.0f}-{args.hi:.0f} C)"
    )
    print(
        f"Discrete chart comparison margin over (V,t): window data = {m_win:.0f} "
    ...
    a1.plot(TEMP_CENTERS[occ], w[occ], "o-", color="#1f6fb2", lw=2, label="learned weighting w(T)")
    a1.axhline(0, color="gray", lw=0.6)
    a1.set_xlabel("temperature T (C)")
    a1.set_ylabel("learned weight (standardized)")
    a1.set_title(
        "Functional regression recovers the UNLISTED\ntemperature-window controller",
        fontweight="bold",
        fontsize=11,
    )
    ...
    a2.set_title(
        "Discrete chart menu: wins for a listed coordinate,\ngoes diffuse for the unlisted window",
        fontweight="bold",
        fontsize=11,
    )
    a2.annotate(
