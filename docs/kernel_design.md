...
We constrain $\ell_d \geq \ell_{\min}$ (typically 0.03) to prevent:
...
- Minimum constraint (0.03) allows sensitivity without overfitting  
- Maximum constraint usually not needed (MLE penalizes too-large values)
...
and $\boldsymbol{\theta} = \{\sigma_f^2, \boldsymbol{\ell}, \sigma_n^2\}$
